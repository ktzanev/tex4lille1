...
\begin{document}
  \begin{tikzpicture}[>=latex]
    ...
    \draw[<->,red] (B.south-|H) -- node[sloped,above] {height} (B.north-|H);
    \draw[<->,red] (B.west|-W) -- node[above] {width} (B.east|-W);

    % title
    \path (M.north) node[red, above, font=\ttfamily] (CSS) {box-sizing\,:\,border-box;};
    \path (CSS.north) node[above, gray, font=\large] {IE box model};
  \end{tikzpicture}
\end{document}
